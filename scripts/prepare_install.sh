...
function hydrate_shared_config {
  ...
  case $DB_TYPE in
     sqlserver)
         ...
         export DB_JDBCURL="jdbc:sqlserver://${DB_SERVER_NAME}.azure-np.hydro.qc.ca:${DB_PORT};database=${DB_NAME};user=${DB_USER}@${DB_SERVER_NAME};Password=${DB_PASSWORD};encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30"
